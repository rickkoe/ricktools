...
def fs_map(group, host_name, volume_name, volume_qty, volume_start, host_qty, volume_count_length):
    group += 1
    volumes_per_group = volume_qty/host_qty
    range_list = []

    ## Original Code that works to break into groups
    # starting_volume = 0
    # for i in range(host_qty):
    ...
    if group == 1:
        ...
        host_extension = '_pool2'
    if volume_qty > 1:
        # Next 2 lines needed for Inmar.   Commented out original code above
        start = int(volume_start)
        end = volume_start + volume_qty - 1
        # return f'for ((i={start+1};i<={end+1};i++)); do j=$(printf "%0{volume_count_length}d" "$i"); svctask mkvdiskhostmap -force -host {host_name}{host_extension} {volume_name}$j; done'
        return f'for ((i={start};i<={end};i++)); do j=$(printf "%0{volume_count_length}d" "$i"); svctask mkvdiskhostmap -force -host {host_name}{host_extension} {volume_name}$j; done'

    else:
        return f'svctask mkvdiskhostmap -force -host {host_name}{host_extension} {volume_name}'

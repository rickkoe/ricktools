    ...
    try:
        ...
        for file_name in os.listdir(input_directory):
            if file_name != '.DS_Store':
                ...
                write_to_doc(output_directory, component_dict, customer_name, cluster_name, code_level, cluster_ip)
...
def write_to_doc(output_directory, component_dict, customer_name, cluster_name, code_level, cluster_ip):
    doc_name = f"{customer_name} {cluster_name} As-Installed.docx"
    doc = Document()
    print(f'\nCreating Document: {doc_name}')
    doc.add_heading(customer_name)
    doc.add_heading(f'{cluster_name} As-Installed Documentation')
    for key, dict_list in sorted (component_dict.items()):
        write_doc(doc, key, dict_list)
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
    doc.save(os.path.join(output_directory,doc_name))
    print(f"Document saved to: {os.path.join(output_directory,doc_name)}")
...
def write_doc(doc, key, data):
    headers = find_max_keys(data)
    
    # This worked in place of using the find_max keys but not ordered correctly
    # # headers = list(set(itertools.chain.from_iterable(data))) 
    
    doc.add_paragraph(headers)
    for elements in data:
        doc.add_paragraph([elements.get(h) for h in headers])
    return doc
